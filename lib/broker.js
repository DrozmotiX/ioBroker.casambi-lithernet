 ...
 *
 * aedes >= 1 is ESM-only and exposes `createBroker` as an async static on the
 * `Aedes` class, so it is pulled in with a dynamic import from this CommonJS module.
 */
class CasambiBroker {
	/**
	 * @param {ioBroker.Adapter} adapter - adapter instance (for logging + info.connection)
	 * @param {object} options - { bind, port, username, password }
	...
	async start() {
		...
		const broker = await Aedes.createBroker(aedesOptions);
		this.aedes = broker;
		...
		return new Promise((resolve, reject) => {
			...
			broker.on('publish', (packet, client) => {
				...
				// Log every incoming gateway message at debug (set instance log level to
				// debug to inspect raw traffic during setup).
