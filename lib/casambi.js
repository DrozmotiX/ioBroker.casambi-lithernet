...
function unhandledShape(topic, cfg) {
	...
	// poll_device sub-topics other than values/propertys are the unmapped element_* button/dimmer
	// families - worth sampling for later mapping even though poll_device itself is a known kind.
	const isElementSub =
		parts[3] === 'poll_device' && parts[5] != null && parts[5] !== 'values' && parts[5] !== 'propertys';
	if (KNOWN_FEEDBACK_KINDS.has(parts[3]) && !isElementSub) {
		return null;
	}
	return parts
		.slice(2)
...
/**
 * Decide which debounced device readback fields to actually write as CONFIRMED (ack:true).
 * Returns only the level/on fields whose value differs from the last confirmed snapshot, so
 * a value is acked exactly once from real gateway data and steady re-polls don't churn.
 *
 * The gateway emits a STALE-then-fresh pair on every scene recall (the old level immediately
 * followed by the new one, ~10ms apart). Debouncing collapses the pair to the last sample;
 * this diff then writes only what really changed. Pass an empty/!confirmed snapshot (e.g. after
 * a command invalidates it) to force a re-confirm even when the value is unchanged.
 *
 * @param {{level?: number, on?: boolean}} pending - the last debounced readback sample
 * @param {{level?: number, on?: boolean}|undefined} confirmed - last confirmed snapshot
 * @returns {{level?: number, on?: boolean}} fields to write (ack:true)
 */
function diffConfirmedReadback(pending, confirmed) {
	const out = {};
	for (const field of ['level', 'on']) {
		if (pending[field] !== undefined && pending[field] !== (confirmed ? confirmed[field] : undefined)) {
...
/**
 * Plan the scene-level recall sequence for a per-device command. A bare scene-level-0 only
 * DEACTIVATES the control scene, so if a foreign scene (e.g. a wall button's scene) is currently
 * active the load falls back to it and stays on. An OFF therefore first GRABS the load onto its
 * own control scene at its current level (no visible change), then sets 0 - "override-then-zero",
 * verified live. A non-zero target already overrides a foreign scene, so it needs only one recall.
 *
 * @param {number} targetDisplay - requested level in display units (0 = off)
 * @param {number} currentDisplay - the load's last known level in display units
 * @returns {number[]} ordered display levels to recall (2 = grab-then-zero, else 1)
 */
function planDeviceLevels(targetDisplay, currentDisplay) {
	if (targetDisplay <= 0 && currentDisplay > 0) {
...
/**
 * Settle-window decision for a device readback while a command is in flight. The gateway re-polls
 * a load at its pre-change level for ~1s after a recall; writing that produced the on/off flip.
 *
 *   'drop'   - within the window and the sample still contradicts the commanded on/off (ignore it)
 *   'accept' - no command pending, the sample reflects the command, or the window has elapsed
 *              (reality wins, so a failed/lost command reverts to the true state)
 *
 * @param {{on?: boolean}} pending - the debounced readback sample
 * @param {{on: boolean, until: number}|undefined} expect - in-flight command target + deadline
 * @param {number} now - current epoch ms
 * @returns {'drop'|'accept'} what to do with the sample
 */
function settleReadback(pending, expect, now) {
	if (!expect || now >= expect.until) {
		return 'accept';
	}
	if (pending.on !== undefined && pending.on !== expect.on) {
		return 'drop';
	}
	return 'accept';
}
...
function parseGet(topic, payload, cfg) {
	...
	switch (kind) {
		...
		case 'poll_device': {
			// ONLY .../poll_device/<N>/values and .../propertys flatten under devices.<N>. Other
			// sub-topics - notably the button/dimmer module element_* families
			// (element_button/pushbutton/slider/onoff/dimmer) - must NOT be flattened, or each of
			// their fields (button_1..8, pushbutton_1..8, slider_1..8, onoff_1..8, dimmer_1..4)
			// becomes a raw state and the device tree explodes. They are returned as unhandled so
			// the sampler can record them for proper button/dimmer mapping later.
			if (parts[4] == null || (parts[5] !== 'values' && parts[5] !== 'propertys')) {
				return null;
			}
			...
			// Derive a read-only on/off (values sub-topic only) so each real device exposes a boolean
			// state alongside its level. A DIMMER carries on/off in `level`; a RELAY/SWITCH keeps
			// `level` at 0 and instead FOLLOWS its control scene, so its `scene` field (non-zero = a
			// scene is currently driving the unit) marks it on (verified live on a switch: scene
			// 0<->103 while level stayed 0). The union also covers a manually-dimmed lamp (scene 0,
			// level up). Monitoring only - the unit has no set topic (consumed e.g. by oikos-connect).
			if (parts[5] === 'values' && typeof cleaned.level === 'number') {
				cleaned.on = cleaned.level > 0 || Number(cleaned.scene) > 0;
			}
