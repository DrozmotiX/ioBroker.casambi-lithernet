...
describe('cloudModel.manualMapFromConfig + resolveControlScene', () => {
	const scenes = model.parseScenes(NETWORK); // 1(multi 1,2,3), 10(dev1), 50(dev1), 99(dev2)

	it('manualMapFromConfig keeps only valid single-member picks, keyed by their device', () => {
		const map = model.manualMapFromConfig(scenes, [{ scene: 50 }, { scene: 99 }]);
		expect(map).to.deep.equal({ 1: 50, 2: 99 }); // scene 50 -> dev 1, scene 99 -> dev 2
	});

	it('manualMapFromConfig ignores multi-member scenes and unknown ids; last wins per device', () => {
		const map = model.manualMapFromConfig(scenes, [{ scene: 1 }, { scene: 777 }, { scene: 10 }, { scene: 50 }]);
		expect(map).to.deep.equal({ 1: 50 }); // scene 1 multi -> dropped, 777 unknown -> dropped, 10 then 50 -> last wins
	});
